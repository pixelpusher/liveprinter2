...
visualiser.setZoom(0.8);
visualiser.setViewX(0);
visualiser.setViewY(0);
visualiser.closeFactor(90);
...
const crossFadeFunc = crossfadePoints(preset.points_per_layer*20);
const crossFadeFunc2 = crossfadePoints(preset.points_per_layer*10);
...
# fan 0 | prime | fan 50 | speed 80 | mov2 pointsFunc() | unretract | wait 20 | fan 10
  ...
  if (gridPoint.z <= preset.height) {
    ...
    if (gridPoint.i > 0 && gridPoint.i % preset.points_per_layer === 0) {
      ...
      else if (layers === 15) 
      {
        ...
        const d = 16*lp.n2mm('c3', '1/2b',preset.bpm);
        ...
        const crossFadeFunc3 = crossfadePoints(preset.points_per_layer*20);

        pointsFunc = ()=>crossFadeFunc3(nextGridPoint(),nextGridPoint3());
      }

      //loginfo(\`layer increase\`);
    }
